...
try {
    ...
    $stmt = $conn->prepare("
        ...
        WHERE c.id = ? AND (c.agent_id = (SELECT id FROM customers WHERE email = ? AND is_agent = 1))
    ");
    
    $stmt->execute([$_GET['commission_id'], $_SESSION['user_email']]);
    $commission = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$commission) {
        die('Commission not found or you do not have permission to view it');
    }
    ...
    $order = [
        ...
        'line_items' => json_encode($items_with_commission),
    ...
    ob_start();
    require_once __DIR__ . '/../templates/commission_invoice.php';
    $html = ob_get_clean();
    ...
    if (isset($_GET['output'])) {
        ...
        header('Content-Disposition: inline; filename="commission_invoice.pdf"');
        echo $dompdf->output();
    }
} catch (Exception $e) {
